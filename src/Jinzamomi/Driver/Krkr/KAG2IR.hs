...
compile :: [Kag] -> IR.Node
compile nodes = IR.Call (IR.Raw "jinzamomi.krkr.onloadKag") [IR.Function ["macros"] (IR.Return (fld' [] [] nodes))]
  where
...
compileKag (KagText text _) = IR.Str text
...
compileKag (KagNewline _) = IR.Raw "macros.__newline"
